...
Phase 1 op point (the fig-1 bad case corrected) — every case inherits this
except the knobs it Varies: `q=8, nu=0.03, ic-amplitude=0.3, ic=sine,
bc=periodic, source=none, n-steps=512, evolution-mode=measure_reprepare,
segment-size=4, phi-modes=8, bond-dim=4, shots=131072, seed=1234,
save-every=4`. (k=4 = paper-like anchor; paper used n-steps=400/k=5. bond-dim
pinned 4 — scales to q16 and already exact for this phi.)

**C0 — idealized SV & seamless t=0 to t=end**
- Purpose: zero-seam floor — pure algorithm error, then + sampling. If bad
  here, seams aren't the story.
- Vary: `evolution-mode=single, shots=[0, 131072]`

**C1 — seam sweep (PRIMARY)**
- Purpose: is seam count the lever? does over-amp die as S→1?
- Vary: `segment-size=[2,4,8,16,32,64,128,256,512]` → S=[256..1]
- Note: k≥32 → Frontier (~9x cost per k doubling).

**C2 — shots ladder × k**
- Purpose: split per-seam shot-noise (∝1/√S) from truncation floor (flat bias).
- Vary: `shots=[16384,65536,131072,524288,1048576]` at `segment-size=[4, k*]`
  (2^14,2^16,2^17,2^19,2^20)
- Note: one TOML per k; KEEP the low end (needed for the fit — see "Shots per
  q"). k* = C1 winner.

**C3 — phi re-test at best k**
- Purpose: does the filter help once seams are optimized? (phi=0 = off, never tried)
- Vary: `phi-modes=[0,8,32,128]` at `segment-size=k*`

**C4 — dial-up (Phase 2, after k*, phi* known)**
- Purpose: scale the tuned setting across qubit count.
- Vary: `q=[4,8,16], segment-size=[k*/2,k*,2k*], phi-modes=<per-q>`;
  shots: q4=131072, q8=131072, q16=134217728 (2^27; 2^17 is ~1000x short at
  q16; 2^27 ≈ q8-parity per-bin accuracy, ~30s of sampling).
- Note: q4 & q8 share one TOML (shots=131072); q16 is a SEPARATE shots=2^27
  TOML. q=4 skips C3 (N=16: phi-modes=8 already Nyquist). Rationale in Background.
...
- Note: no FTCS/sim reference exists at q32 → deliverable is
  physics-consistency + trend-vs-q, not relL2. See "Beyond q=16" in Background.

## Skeleton to clone

```toml
# CH q=8 seam sweep — segment-size is the only axis. All else = fig-1 baseline.
[global]
_output_dir = "~/q8020"
...
"--segment-size" = [2, 4, 8, 16, 32, 64, 128, 256, 512]   # THE swept lever (k)
# NOTE: no "--propagator" line — flag removed.
```
...
q + 2 (bond) + min(k,q) (heat ancillas, one per step in a segment). At k=4 that
is q+6 → q32 = 38 qubits = 4.4 TB. Even the classical MPS prep chokes first (it
